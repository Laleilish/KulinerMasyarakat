...
<header class="h-16 px-4 md:px-8 flex items-center justify-between lg:justify-end sticky top-0 z-30 bg-cream-light shadow-navbar-admin">
    ...
    <div class="flex items-center gap-4">
        ...
        <!-- Profile: Desktop Dropdown / Mobile triggers slide-in panel -->
        <div class="relative" x-data="{ profileOpen: false, settingsOpen: false }" @click.away="profileOpen = false">
            {{-- Avatar button — desktop opens dropdown, mobile opens slide-in --}}
            <button
                @click="window.innerWidth >= 1024 ? (profileOpen = !profileOpen) : window.adminProfilePanel.open()"
                class="flex items-center gap-2 hover:opacity-80 transition-opacity bg-transparent border-none cursor-pointer focus:outline-none">
            ...
            <div x-show="profileOpen" 
                 ...
                 x-transition:leave-start="transform opacity-100 scale-100"
                 x-transition:leave-end="transform opacity-0 scale-95"
                 class="hidden lg:block absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50"
                 x-cloak>
...
{{-- ===== MOBILE SLIDE-IN PROFILE PANEL (admin) ===== --}}
{{-- Overlay --}}
<div id="adminProfileOverlay"
     class="fixed inset-0 bg-black/40 z-[998] opacity-0 pointer-events-none transition-opacity duration-300 lg:hidden">
</div>

{{-- Slide-in Panel --}}
<div id="adminProfilePanel"
     x-data="{ settingsOpen: false }"
     class="fixed top-0 right-0 h-full w-75 bg-cream-bg z-[999] shadow-2xl
            translate-x-full transition-transform duration-300 ease-in-out
            flex flex-col rounded-l-2xl overflow-hidden lg:hidden">

    {{-- Header: Avatar + Info + Close --}}
    <div class="px-6 pt-6 pb-5">
        {{-- Close Button --}}
        <div class="flex justify-end mb-4">
            <button id="adminProfilePanelClose"
    ...
    {{-- Menu Items --}}
    <nav class="flex flex-col px-4 py-4 gap-1 flex-1 overflow-y-auto relative">
        ...
        <a href="{{ route('home') }}"
           class="flex items-center justify-between px-3 py-4 rounded-xl hover:bg-black/5 transition-colors no-underline group">
            <div class="flex items-center gap-4">
                ...
                <span class="font-semibold text-dark text-sm">Kembali Ke KUMAR</span>
            </div>
            ...
            <div class="flex items-center justify-between py-3">
                ...
                <div class="flex items-center gap-2">
                    <button type="button"
                            ...
                            class="text-sm bg-transparent border-none cursor-pointer transition-colors hover:text-dark">ID</button>
                    <span class="text-sm text-muted">|</span>
                    <button type="button"
                            @click="$store.i18n.setLocale('EN')"
                            :class="$store.i18n.locale === 'EN' ? 'font-bold text-dark' : 'text-muted'"
                            class="text-sm bg-transparent border-none cursor-pointer transition-colors hover:text-dark">EN</button>
                </div>
            </div>
        </div>
