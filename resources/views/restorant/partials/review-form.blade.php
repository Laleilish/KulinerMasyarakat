<div x-show="showForm" x-collapse class="mb-6">
    <form action="{{ route('reviews.store', $restaurant->id) }}" method="POST" enctype="multipart/form-data"
          ...
          x-data="{ rating: {{ old('rating', 0) }}, reviewPhotoName: '' }">
        @csrf
        ...
        <div class="mb-5">
            ...
            <label class="inline-flex w-48 justify-center px-4 pt-4 pb-5 border-2 border-teal-100 border-dashed rounded-xl bg-gray-50 hover:bg-teal-50/50 transition-colors cursor-pointer group">
                <div class="space-y-1 text-center flex flex-col items-center">
                    ...
                    <p x-show="!reviewPhotoName" class="text-xs text-teal-600 font-bold">Upload Foto</p>
                    <p x-show="reviewPhotoName" x-text="reviewPhotoName" class="text-xs text-teal-600 font-bold truncate max-w-[200px]"></p>
                    <input name="photo" type="file" accept="image/jpg,image/jpeg,image/png" class="sr-only"
                           @change="reviewPhotoName = $event.target.files[0]?.name || ''">
                </div>
            </label>
        </div>
